...
$propertiesDir = Join-Path $projectDir "My Project"

# Ensure My Project directory exists (VB.NET uses "My Project" instead of "Properties")
if (-not (Test-Path $propertiesDir)) {
    Write-Host "Creating My Project directory..."
...
$templateSource = Join-Path $scriptDir "SharedVersionInfo.vb.tt"
...
$existingItem = $projectXml.SelectSingleNode("//ns:None[@Include='My Project\VersionInfo.tt']", $nsManager)
...
if (-not $existingItem) {
    ...
    $noneElement.SetAttribute("Include", "My Project\VersionInfo.tt")
    ...
    $lastGenOutputAttr.InnerText = "VersionInfo.vb"
    ...
    $compileElement.SetAttribute("Include", "My Project\VersionInfo.vb")
...
if (-not $existingImport) {
    ...
    # Add after the VisualBasic.targets import
    $vbImport = $projectXml.SelectSingleNode("//ns:Import[contains(@Project, 'VisualBasic.targets')]", $nsManager)

    if ($vbImport) {
        $projectXml.Project.InsertAfter($importElement, $vbImport)
    }
    else {
    ...
    $execElement.SetAttribute("Command", 'if exist "$(ProjectDir)My Project\VersionInfo.tt" "$(DevEnvDir)TextTransform.exe" -out "$(ProjectDir)My Project\VersionInfo.vb" "$(ProjectDir)My Project\VersionInfo.tt"')
...
if not exist "My Project\VersionInfo.tt" (
    echo Error: VersionInfo.tt template not found in My Project folder.
    exit /b 1
)
...
"%TEXTTRANSFORM%" -out "My Project\VersionInfo.vb" "My Project\VersionInfo.tt"
...
1. The versioning system uses a T4 template (`My Project\VersionInfo.tt`) that generates a VB.NET file (`My Project\VersionInfo.vb`) containing the assembly version attributes.
...
3. Generate an updated `VersionInfo.vb` file
